...
# =====================================================
# 0) CONFIG – Relative Paths (portable for GitHub)
# =====================================================
...
# =====================================================
# 3) REMOVE DUPLICATES & ENFORCE UNIQUE IDs
# =====================================================
...
def detect_outliers_iqr(data, column):
    """Detects outliers using IQR and creates <column>_outlier flag."""
...
rules = [
    ...
    ("hire_before_termination",
     ...
     "HireDate must be <= TerminationDate"),

    ("junior_not_manager",
     lambda df_: ~((df_["JobLevel"] == 1) & (df_["JobRole"].str.lower().str.contains("manager"))),
     ["JobLevel", "JobRole"],
     "JobLevel 1 cannot have a manager role")
]
